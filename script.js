...
function fn2() {
    ...
    .then((id) => getid(id, date))
    .catch((error) => {
        console.error('Error fetching location data:', error);
    });

    const getid = (data3, date) => {
        ...
        .then((id) => generateHTML3(id, mydate2))
        .catch((error) => {
            console.error('Error fetching hotel data:', error);
        });
    }

    const generateHTML3 = (data4, mydate2) => {
        var today = new Date();
        var html3 = `<div>Your chosen holiday is on ${date}</div><br>`;
        if (today > mydate2) {
